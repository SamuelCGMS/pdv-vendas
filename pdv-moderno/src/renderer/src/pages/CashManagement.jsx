  ...
  return (
    <div className="flex-col h-full w-full" style={{ padding: '32px', overflowY: 'auto' }}>
      ...
      {activeTab === 'fechamento' && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.8)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(4px)' }}>
          <div className="card glass flex-col" style={{ width: '500px', padding: '32px', border: '2px solid var(--primary)', alignItems: 'center', boxShadow: '0 10px 40px rgba(0,0,0,0.5)' }}>
            <h2 style={{ fontSize: '2.5rem', color: 'var(--text-primary)', marginBottom: '16px', fontWeight: 'bold' }}>Leitura Z (Fechamento)</h2>
            <div style={{ width: '100%', backgroundColor: '#fff', padding: '24px', borderRadius: 'var(--radius-md)', marginBottom: '32px', fontFamily: '"Courier New", Courier, monospace', fontSize: '1.2rem', color: '#000', boxShadow: 'inset 0 0 10px rgba(0,0,0,0.1)' }}>
              <div style={{ textAlign: 'center', fontWeight: 'bold', marginBottom: '16px', fontSize: '1.5rem' }}>GRAVITY MERCADO</div>
              ...
              <div className="flex justify-between" style={{ borderTop: '1px dashed #000', paddingTop: '8px', marginTop: '8px', fontWeight: 'bold' }}>
            ...
            <div className="flex gap-4 w-full">
              ...
              <button className="btn btn-danger" style={{ flex: 2, fontWeight: 'bold', padding: '16px' }} onClick={onCloseRegister}>Imprimir Z e Encerrar Turno</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="flex justify-between items-center" style={{ marginBottom: '32px' }}>
        ...
        <button className="btn btn-danger" style={{ fontWeight: 'bold', padding: '16px 24px', fontSize: '1.2rem' }} onClick={() => setActiveTab('fechamento')}>
      ...
      {activeTab === 'resumo' && (
        <div className="flex gap-6 h-full">
          <div className="card glass flex-col" style={{ flex: 1, padding: '40px' }}>
            ...
            <div style={{ fontSize: '4rem', fontWeight: 'bold', color: 'var(--success)', letterSpacing: '-1px' }}>
            ...
            <div style={{ marginTop: '32px', padding: '32px', backgroundColor: 'var(--surface-100)', borderRadius: 'var(--radius-md)' }}>
              <div className="flex justify-between" style={{ marginBottom: '16px', fontSize: '1.2rem' }}>
                ...
                <span style={{ fontWeight: 'bold', color: 'var(--text-primary)' }}>R$ {(operator.initialCash || 0).toFixed(2)}</span>
              </div>
              <div className="flex justify-between" style={{ marginBottom: '16px', fontSize: '1.2rem' }}>
                <span style={{ color: 'var(--text-secondary)' }}>Vendas Líquidas em Dinheiro</span>
                <span style={{ fontWeight: 'bold', color: 'var(--success)' }}>+ R$ {(cashInDrawer - (operator.initialCash || 0) - movements.reduce((acc, m) => acc + (m.type === 'suprimento' ? m.amount : -m.amount), 0)).toFixed(2)}</span>
              </div>
              <div className="flex justify-between" style={{ marginBottom: '16px', color: 'var(--danger)', fontSize: '1.2rem' }}>
                <span>Sangrias Realizadas</span>
                <span style={{ fontWeight: 'bold' }}>- R$ {movements.filter(m => m.type === 'sangria').reduce((a, m) => a + m.amount, 0).toFixed(2)}</span>
              </div>
              <div className="flex justify-between" style={{ color: 'var(--success)', fontSize: '1.2rem' }}>
                <span>Suprimentos Adicionados</span>
                <span style={{ fontWeight: 'bold' }}>+ R$ {movements.filter(m => m.type === 'suprimento').reduce((a, m) => a + m.amount, 0).toFixed(2)}</span>
              </div>
            </div>
          </div>
                ...
                <div className="flex-col gap-4">
                  ...
                  {[...movements].reverse().map(m => (
                     <div key={m.id} className="flex justify-between items-center" style={{ padding: '24px', backgroundColor: 'var(--surface-100)', borderRadius: 'var(--radius-sm)', borderLeft: `6px solid ${m.type === 'sangria' ? 'var(--danger)' : 'var(--success)'}` }}>
                       <div>
                         <div style={{ fontWeight: 'bold', textTransform: 'uppercase', fontSize: '1.1rem', color: m.type === 'sangria' ? 'var(--danger)' : 'var(--success)', marginBottom: '4px' }}>
                           {m.type}
                         </div>
      ...
      {(activeTab === 'nova_sangria' || activeTab === 'novo_suprimento') && (
        <div className="flex justify-center" style={{ paddingTop: '32px' }}>
          <div className="card glass flex-col items-center justify-center" style={{ padding: '40px', width: '500px', border: `2px solid ${activeTab === 'nova_sangria' ? 'var(--danger)' : 'var(--success)'}` }}>
            <h2 style={{ fontSize: '1.8rem', marginBottom: '8px', color: activeTab === 'nova_sangria' ? 'var(--danger)' : 'var(--success)', fontWeight: 'bold' }}>
            ...
            <div className="flex items-center" style={{ backgroundColor: 'var(--surface-100)', color: 'var(--text-primary)', borderRadius: 'var(--radius-md)', padding: '0 20px', border: '2px solid var(--border-focus)', marginBottom: '16px', width: '100%' }}>
              <span style={{ fontSize: '2rem', fontWeight: 'bold', marginRight: '16px', color: 'var(--text-secondary)' }}>R$</span>
              <input 
                autoFocus
                type="number" 
                value={amountInput}
            ...
            <div style={{ width: '100%', marginBottom: '24px' }}>
              <label style={{ color: 'var(--text-secondary)', marginBottom: '8px', display: 'block', fontSize: '1rem', fontWeight: 'bold', textAlign: 'left' }}>
            ...
            <button className={`btn ${activeTab === 'nova_sangria' ? 'btn-danger' : 'btn-success'}`} style={{ width: '100%', padding: '16px', fontSize: '1.2rem', fontWeight: 'bold' }} onClick={() => handleAction(activeTab === 'nova_sangria' ? 'sangria' : 'suprimento')}>
